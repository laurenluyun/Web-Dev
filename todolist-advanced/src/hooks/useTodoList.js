...
export function useTodoList(currentList) {
  // each function uses the mutate function to update the data after performing an action. 
  ...
  return {
    ...
    async newItem(newItemName) {
      ...
      return await mutate(
        ...
        }),
        {
          ...
          // optimisticData is used to update the data optimistically, providing a smooth user 
          // experience by updating the UI before the actual response is received. 
    ...
    async deleteItem(itemToDelete) {
      return await mutate(
        ...
        {
          // The populateCache option is set to false to ensure that a refetch is triggered after an action, which helps maintain data consistency.
          populateCache: false, // because our putter doesn't return the new item so we want to refetch it instead
          optimisticData: oldData => ({
            ...oldData,
